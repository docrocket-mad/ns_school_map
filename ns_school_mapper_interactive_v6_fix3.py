#!/usr/bin/env python3
"""
ns_school_mapper_interactive_v6_fix3.py

Stable build:
- No Python f-string collisions with JS ${...} (JS lives in a plain string)
- Filters (Status, District, optional Group/Board), search, add-on-map
- Status editing with live color updates (CircleMarkers, no external CDNs)
- NS-biased geocoding (Nominatim bbox + country), cache, failed_geocodes export
- Honors existing lat/lon from your Excel and seeds cache with them

Run:
  python ns_school_mapper_interactive_v6_fix3.py --input "2024_2025 Elementary Schools.xlsx" --output "ns_schools_map_editable.html" --regeocode-failed --min-delay-seconds 2.0
"""
...
def load_workbook(input_path: Path, district_col: Optional[str]) -> pd.DataFrame:
    ...
    # Silence future dtype warning by excluding truly empty frames
...
def build_map(df: pd.DataFrame, output_html: Path):
    ...
    # ---- Build JS in two parts to avoid Python f-string parsing of ${...} ----
    js_head = f"""
<script>
...
"""
    ...
    # JS body contains ${...} template literals — keep as PLAIN string (no f prefix)
    js_body = """
...
"""
    ...
    # If there's a group column, build the little form field; otherwise empty
    group_field_html = (
    ...
    # Replace placeholder in JS body safely
    js_body = js_body.replace("__GROUP_FIELD_HTML__", group_field_html)
...
def main():
    ...
    geolocator = Nominatim(user_agent="ns_schools_mapper_v6_fix3")

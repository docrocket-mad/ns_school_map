#!/usr/bin/env python3
"""
ns_school_mapper_interactive_v6_fix.py

Features:
- Filters: Status, District, optional Group/Board (if your sheet has a column named Group/Board/System)
- Search: keyword over School + Address, with "Zoom to first"
- Add-on-Map: toggle add mode, click to drop a new school, fill mini-form, Save (included in Download CSV)
- Status editing in popups; colored CircleMarkers update live (no external CDNs)
- Nova Scotia–biased geocoding (Nominatim with bbox + country code), caching, and failed_geocodes export
- Honors existing lat/lon in your Excel (so manual fixes persist)

Run:
  python ns_school_mapper_interactive_v6_fix.py --input "2024_2025 Elementary Schools.xlsx" --output "ns_schools_map_editable.html" --regeocode-failed --min-delay-seconds 2.0
"""
...
STATUS_COL = "Status"
GROUP_COLS = ["Group", "Board", "System"]   # optional column names you might use
...
def build_map(df: pd.DataFrame, output_html: Path):
    ...
    # Build the optional Group/Board field HTML in Python (to avoid f-string collisions)
    group_field_html = ""
    if group_col:
        group_field_html = (
    ...
    # Controls panel (safe Python f-string — no JS ${...} here)
    controls_html = f"""
    ...
    # Main JS (no ${...} in Python space except harmless `${lat}` `${lon}` inside backticks)
    js = f"""
    <script>
...
def main():
    ...
    geolocator = Nominatim(user_agent="ns_schools_mapper_v6_fix")

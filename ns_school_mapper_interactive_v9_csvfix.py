#!/usr/bin/env python3
"""
ns_school_mapper_interactive_v9_csvfix.py

What's inside:
- Auto-detect input type: CSV or Excel workbook (multi-sheet).
- Map + List dashboard:
  * Filters: Status (None/Recent/Current), Group, Search
  * Toggleable List View synced with filters/search
  * Click a list item -> zoom to pin + open popup
- Add Mode: type address (client geocode) or click map; choose custom Group; set Status.
- Move pin: "Move pin" then click map.
- Delete pin + Undo.
- CSV download of the current in-memory dataset (for persistence between sessions).
- Python-side geocoding with NS bounding box + caching; exports failed geocodes CSV.

Usage examples:
  python ns_school_mapper_interactive_v9_csvfix.py --input "2024_2025 Elementary Schools.xlsx" --output "ns_schools_map_editable.html" --regeocode-failed --min-delay-seconds 2.0
  python ns_school_mapper_interactive_v9_csvfix.py --input "ns_schools_map_editable.csv" --output "ns_schools_map_editable.html" --no-cache
"""
...
def load_workbook(input_path: Path, district_col: Optional[str]) -> pd.DataFrame:
    """Read all sheets; add District=sheet name if missing; concat."""
...
def derive_status(row: pd.Series) -> str:
    """Row-based derivation (used in main when building dataset)."""
...
"""
    ...
    # ---- JS body (raw string; ${...} kept for JS template literals) ----
    js_body = r"""
...
def main():
    ...
    # Normalize addresses for geocoding (used if we need to geocode)
    df["__full_addr__"] = df[args.address_col].astype(str).map(normalize_address)

    # Cache + geocoder
    cache = {} if args.no_cache else load_cache(cache_path)
    geolocator = Nominatim(user_agent="ns_schools_mapper_v9_csvfix")
    ...
    # Normalize status to the new scheme and save snapshot CSV
    df[STATUS_COL] = df.apply(derive_status, axis=1)

#!/usr/bin/env python3
"""
ns_school_mapper_interactive_v8.py

Changes in this version:
- Status values are now: None, Recent, Current  (case-insensitive; legacy values mapped to these)
- "District" filter replaced by "Group". If no `Group` column is present, we derive it from
  Group/Board/System/District (first present takes precedence).
- Add Mode includes a free-text Group field so you can add custom groups.
- Delete now supports Undo (panel button: "Undo Delete").
- Move pin: from a popup, click "Move pin", then click on the map to set the new location.

Keeps:
- Search, Zoom to first, Download CSV
- Client-side geocoding for typed addresses (bounded to Nova Scotia)
- Python-side NS-biased geocoding for initial dataset, caching, and failed_geocodes export

Run:
  python ns_school_mapper_interactive_v8.py --input "2024_2025 Elementary Schools.xlsx" --output "ns_schools_map_editable.html" --regeocode-failed --min-delay-seconds 2.0
"""
...
# ---- Config ----
STATUS_COL = "Status"  # stored column name
# Canonical UI statuses (lowercase keys for data; display capitalized in UI)
STATUS_VALUES = ["none", "recent", "current"]
COLOR = {
    "none": "#808080",
    "recent": "#1f77b4",
    "current": "#2ca02c",
}
# Columns we might use to derive Group if `Group` not present
...
def derive_status(row: pd.Series) -> str:
    """Map any legacy values to {none, recent, current}."""
    raw = str(row.get(STATUS_COL, "")).strip().lower()
    if raw in STATUS_VALUES:
        return raw
    # Legacy fallbacks
    if raw in {"active", "both", "current"}:
        return "current"
    if raw in {"recent", "relationship", "worked"}:
        return "recent"
    # Derive from boolean-ish columns if present
...
def pick_group_column(df: pd.DataFrame) -> str:
    for c in GROUP_CANDIDATES:
        if c in df.columns:
            return c
    # fallback create Group from District if exists, else blank
    if "District" in df.columns:
        df["Group"] = df["District"]
        return "Group"
    # else create empty Group column
    df["Group"] = ""
    return "Group"
...
def build_map(df: pd.DataFrame, output_html: Path):
    ...
    needed = ["School","Address","lat","lon"]
    for col in needed:
        if col not in df.columns:
            df[col] = ""

    # Normalize/ensure Group column exists
    ...
    if STATUS_COL not in df.columns:
        ...
        df[STATUS_COL] = df[STATUS_COL].astype(str).str.lower().map(derive_status)
    ...
    # Controls UI (f-string safe)
    controls_html = f"""
    <div id="panel" style="position: fixed; top: 20px; left: 20px; z-index: 9999; background: white; padding: 12px; border: 1px solid #ccc; border-radius: 10px; box-shadow: 0 2px 6px rgba(0,0,0,0.15); font-family: system-ui, sans-serif; font-size: 13px; width: 320px; max-height: 80vh; overflow:auto;">
    ...
    # ---- JS split: head (needs Python vars) + body (plain) to avoid ${...} f-string issues ----
    js_head = f"""
<script>
const COLOR={{none:"#808080",recent:"#1f77b4",current:"#2ca02c"}};
const DATA={js_data};               // Each record: {{School, Address, Group, lat, lon, Status}}
let addMode=false;
...
const GROUPS={js_groups};           // initial distinct values
"""
...
function undoDelete() {
  ...
  if (action.type === 'delete') {
    // Insert back close to original index (or at end if out of range)
...
function applyFilters(map) {
  ...
  const groups = new Set(getSelectedMulti('groupSel'));
...
function downloadCSV() {
  ...
  // Always include these headers in a stable order
  const base = ["School","Address","Group","lat","lon","Status"];
  const headers = base;
...
function mapClickHandler(e) {
  ...
  const map = findMap();
  if (moveIndex !== null) {
    // Move the selected pin to this click
    const idx = moveIndex;
    moveIndex = null;
    if (DATA[idx]) {
...
function openAddForm(lat, lon, geocodedLL, usedAddress, usedGroup) {
  ...
  form.querySelector('#f_save').onclick = () => {
    ...
    // update Group filter list with custom added group if any
    if (rec.Group && !GROUPS.includes(rec.Group)) {
...
function init() {
  ...
  document.getElementById('btnGeocode').onclick = async () => {
    ...
    if (ll) {
      ...
      _map.setView([ll.lat, ll.lon], 14);
    } else {
      const base = lastClickLL ? {lat: lastClickLL.lat, lon: lastClickLL.lng} : {lat: _map.getCenter().lat, lon: _map.getCenter().lng};
...
# ---------- Main ----------
def main():
    parser = argparse.ArgumentParser(description="Editable NS schools map (filters, search, add, move, delete+undo).")
    ...
    geolocator = Nominatim(user_agent="ns_schools_mapper_v8")
    ...
    # Normalize status into the new scheme
    df[STATUS_COL] = df.apply(derive_status, axis=1)

    # Save a reference CSV (what went into the map)
    out_csv = output_html.with_suffix(".csv")
    df.to_csv(out_csv, index=False)

    # Build map using just rows that have lat/lon

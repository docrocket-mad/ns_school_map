#!/usr/bin/env python3
"""
ns_school_mapper_interactive_v9_2.py

Adds:
- Phone + Email in pin popups.
- Phone + Email columns included in both CSV downloads (All and Filtered).
- Auto-detect input type (CSV or Excel).
- Aggregates multiple email-like columns into a single "Email" field.

Keeps:
- Map + List dashboard (filters: Status/Group/Search; click list → zoom to pin).
- Add by address (client geocode) or map click; custom Group; Status.
- Move pin; Delete pin + Undo.
- NS-biased geocoding with cache; failed-geocode export.
...
# Likely column names for phone & email; we'll also scan using substring matches.
PHONE_CANDIDATES = ["Phone", "Phone Number", "PhoneNumber", "Main Phone", "School Phone", "Telephone"]
EMAIL_CANDIDATES = [
    "Email", "Email Address", "School Email", "General Email", "Contact Email",
    "Admin Email", "Administrator Email", "Office Email", "Main Email",
    "Principal Email", "Vice Principal Email", "Secretary Email"
]
...
# ---------- Phone/Email helpers ----------
def pick_phone_column(df: pd.DataFrame) -> Optional[str]:
    # exact candidates first
    for name in PHONE_CANDIDATES:
        if name in df.columns:
            return name
    # then any col containing 'phone' or 'tel'
    for c in df.columns:
        cl = c.lower()
        if "phone" in cl or "tel" in cl:
            return c
    return None

def pick_email_columns(df: pd.DataFrame) -> List[str]:
    cols = []
    # exact candidate names if present
    for name in EMAIL_CANDIDATES:
        if name in df.columns:
            cols.append(name)
    # any column with 'email' in name
    for c in df.columns:
        if "email" in c.lower() and c not in cols:
            cols.append(c)
...
def aggregate_emails(row: pd.Series, email_cols: List[str]) -> str:
    items = []
    for c in email_cols:
        v = row.get(c, "")
...
def build_map(df: pd.DataFrame, output_html: Path):
    ...
    # Determine phone/email columns and create unified "Phone" & "Email"
    phone_col = pick_phone_column(df)
    email_cols = pick_email_columns(df)

    view = df.dropna(subset=["lat","lon"]).reset_index(drop=True).copy()
    if phone_col:
        view["Phone"] = view[phone_col].fillna("").astype(str)
    else:
        view["Phone"] = ""
    if email_cols:
        view["Email"] = view.apply(lambda r: aggregate_emails(r, email_cols), axis=1)
    else:
        view["Email"] = ""

    cols = ["School","Address",group_col,"Phone","Email","lat","lon",STATUS_COL]
    view = view[cols].rename(columns={group_col: "Group"})

    import json
    js_data = json.dumps(view.to_dict(orient="records"))
    groups = sorted([x for x in view["Group"].dropna().astype(str).unique() if str(x).strip() != ""])
    ...
    # ---- JS head (embed Python vars) ----
    js_head = f"""
<script>
const COLOR={{none:"#808080",recent:"#1f77b4",current:"#2ca02c"}};
const DATA={js_data};  // {{School, Address, Group, Phone, Email, lat, lon, Status}}
...
# ---------- Main ----------
def main():
    parser = argparse.ArgumentParser(description="Editable NS schools map (phone/email in popups; CSVs include phone/email; list view; add/move/delete; filtered CSV).")
    ...
    # Normalize status to the new scheme and save snapshot CSV (original columns + lat/lon + Status)
    df[STATUS_COL] = df.apply(derive_status, axis=1)
    out_csv = output_html.with_suffix(".csv")
    df.to_csv(out_csv, index=False)

    # Build map from the geocoded subset

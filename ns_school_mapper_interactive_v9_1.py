#!/usr/bin/env python3
"""
ns_school_mapper_interactive_v9_1.py

Adds:
- "Download Filtered CSV" button to export ONLY the rows currently visible
  under filters/search (the same set you see in the list and on the map).

Keeps:
- Auto-detect input (CSV or Excel workbook w/ multiple sheets)
- Map + List dashboard: Status/Group/Search filters; list sync; click-to-zoom
...
Run:
  python ns_school_mapper_interactive_v9_1.py --input "ns_schools_map_editable.csv" --output "ns_schools_map_editable.html" --no-cache
  python ns_school_mapper_interactive_v9_1.py --input "2024_2025 Elementary Schools.xlsx" --output "ns_schools_map_editable.html" --regeocode-failed --min-delay-seconds 2.0
"""
...
# ---------- Main ----------
def main():
    parser = argparse.ArgumentParser(description="Editable NS schools map (filters, search, list view, add, move, delete+undo; filtered CSV export).")
    ...
    geolocator = Nominatim(user_agent="ns_schools_mapper_v9_1")
